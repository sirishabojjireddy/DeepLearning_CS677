...
attk = BBattk(main_epochs=main_epochs,num_nodes=num_nodes, bb_optimizer=bb_optimizer, bb_epochs=bb_epochs)
...
bb_idxs = attk.get_idxs(bb_min_val, bb_max_val, bb_idxs_num)
bb_data = main_data[bb_idxs]

bb_labels = attk.get_bb_labels(bb_data, target_model)

test_data, test_labels = attk.get_non_bb_data(main_data, main_labels, bb_idxs)
...
bb_model = attk.get_bb_model(inp_shape=(32,32,3))

bb_model = attk.train_adv(bb_data, bb_labels, bb_model, target_model, test_data=test_data, test_labels=test_labels, check_acc=True)
